...
# 2. Figure out what data can be used in mutiple recipes and put them into different tables and which data is unique to each recepie
- Author
- Recipe Type
- Prep Time
- Cook Time
...
- a recipe can have only have one author
    - one-to-many relaationship with the recipes table
- could create a authors table then include a foreign key, author_id, column in the recipes table, which links each recipe back to a specific author    
    ### author
...
## Recipe Type
- an recipe type can belong to many recipes, so we do not need to duplicate the recipe types
...
- total time will be calculated dynamically from the prep and cook times
    ...
    - Use a SQL query to compute the total time, saving storage space and ensuring the value is always up-to-date
    - the recommended approach for most applications

## Ingredients
- ingredients can be used in mutiple recipes, so we do not need to duplicate all those values
...
## Recipe Table
- recipe name and description will different for each recipe so we need to store both those values
